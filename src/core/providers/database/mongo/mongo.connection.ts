  ...
  implements IMongoConnection<T>
{
  ...
  async insert(objects: T[]): Promise<T[]> {
    try {
      const docs: Array<T> = [];
  ...
  async queryOne<U>(
    ...
    try {
      // When using lean(), Mongoose returns plain JSON objects instead of memory and resource-heavy documents. It makes queries faster and less expensive on the CPU.
      ...
      if (leanEnabled) {
        // When using Promises in combination with Mongoose async operations, note that Mongoose queries are not Promises.
